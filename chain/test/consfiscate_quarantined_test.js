...
describe("Confiscate and Quarantine", function () {
    ...
    before('deployment', async () => {
        ...
        ;({gate, token, confiscateToken, addressControlStatus} = await deploy.base(web3, solc(__dirname, '../solc-input.json'), DEPLOYER, OPERATOR))
    })
    ...
    context("Confiscate.", async () => {
        it('Operator can confiscate if-and-only-if the address is frozen and function enabled', async() => {
            await send(gate, OPERATOR, enableConfiscate)
            await send(gate, OPERATOR, mint, CUSTOMER, 1000)
            await send(addressControlStatus, OPERATOR, "freezeAddress", CUSTOMER)
            await send(gate, OPERATOR, confiscate, CUSTOMER, 1000)
            expect(await call(token, "balanceOf", CUSTOMER)).eq(0)
            expect(await call(token, "balanceOf", OPERATOR)).eq(1000)   //OPERATOR is QuarantineAddress
        ...
        it('Cannot confiscate when the function is turned off', async() => {
            ...
            expect(await call(token, "balanceOf", OPERATOR)).eq(0)   //OPERATOR is QuarantineAddress
        })
    ...
    context("UnConfiscate.", async () => {
        it('Operator can unConfiscate if-and-only-if the address is unFrozen and function enabled', async() => {
            await send(gate, OPERATOR, enableConfiscate)
            await send(gate, OPERATOR, mint, CUSTOMER, 1000)
            await send(addressControlStatus, OPERATOR, "freezeAddress", CUSTOMER)
            await send(gate, OPERATOR, confiscate, CUSTOMER, 1000)
            ...
            await send(gate, OPERATOR, unConfiscate, CUSTOMER, 1000)
            expect(await call(token, "balanceOf", CUSTOMER)).eq(1000)
            expect(await call(token, "balanceOf", OPERATOR)).eq(0)   //OPERATOR is QuarantineAddress
        })
    ...
    context("setConfiscateCollector", async() => {
        it('SystemAdmin can change confiscate address', async() => {
            ...
            await send(gate, OPERATOR, mint, CUSTOMER, 1000)
            await send(addressControlStatus, OPERATOR, "freezeAddress", CUSTOMER)
            await send(gate, OPERATOR, setConfiscateCollector, CONFISCATE_COLLECTOR)
            await send(gate, OPERATOR, confiscate, CUSTOMER, 1000)
            expect(await call(token, "balanceOf", CUSTOMER)).eq(0)
            expect(await call(token, "balanceOf", OPERATOR)).eq(0)   //OPERATOR is QuarantineAddress
